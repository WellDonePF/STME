...
def fun1():
    all_file_path = [
        f"/cpfs/user/wangpengfei2/project/InternVL/attn_score_analysis/internvl2-2B/{args.task_type}/info/pred0.json",
    ]
    all_dir_path = [
        f"/cpfs/user/wangpengfei2/project/InternVL/attn_score_analysis/internvl2-2B/{args.task_type}/bench_res{num}"
            for num in range(3)
    ]
    all_res = []
...
def fun2():
    ...
    for i in range(len(real_scores)):
        ...
        if dataset_name == "ImageNeedleInAHaystack":
            a += 1
            if item["attn_img_num"] == item["target_id"]:
                b += 1
        all_res.append(item)
    all_res.sort(key=lambda x: (x['dataset_name'], x['sample_id']))
...
if __name__ == "__main__":
    ...
    for fun_num in args.function:
        if int(fun_num) == 1:
            ...
            cal_matrix()
    print(len(os.listdir(f"/cpfs/user/wangpengfei2/project/InternVL/attn_score_analysis/internvl2-2B/{args.task_type}/scores")))
