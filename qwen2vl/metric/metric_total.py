...
def merge_analysis_file():
    ...
    open(os.path.join(args.save_file_path, f"total_analysis_{args.metric}.json"),'w',encoding="utf-8"
        ).write(json.dumps(all_res,indent=4))


# 计算混淆矩阵
def cal_matrix():
    analysis_file_path = os.path.join(args.save_file_path, f"total_analysis_{args.metric}.json")
    ...
    for last_N in args.N:
        ...
        metric_value[save_key] = (cm[1, 1] + cm[0, 1]) / len(is_right)
...
def plot_figure(metric_values, acc):
    ...
    plt.savefig(
        os.path.join(args.save_figure_path, f"total_metric_{args.task_type}_{args.model_name}.pdf"),
        format='pdf', dpi=3000, bbox_inches='tight')
    plt.close()  # 关闭图形以释放内存
    open(os.path.join(args.save_file_path, f"total_result.json"),'w',encoding="utf-8"
        ).write(json.dumps(all_res, indent=4))
    print(all_res)
