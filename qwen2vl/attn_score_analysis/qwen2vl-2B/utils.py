...
def fun1():
    all_file_path = [
        # f"/cpfs/user/wangpengfei2/project/Qwen2VL/attn_score_analysis/qwen2vl-2B/{args.task_type}/info/pred0.json",
    ]
    all_dir_path = [
        f"/cpfs/user/wangpengfei2/project/Qwen2VL/attn_score_analysis/qwen2vl-2B/{args.task_type}/bench_res{num}"
            for num in range(1)
    ]
    all_res = []
...
def cal_matrix():
    ...
    for item in analysis:
        ...
        # if is_right[-1] == 1 and attn_index[-1] == 0:
            # print(item["dataset_name"], item["sample_id"])
    ...
    print(cm)
    precision_0 = cm[0, 0] / (cm[0, 0] + cm[0, 1])  # 类别 0 的精度
    precision_1 = cm[1, 1] / (cm[1, 1] + cm[1, 0])  # 类别 1 的精度
    recall_0 = cm[0, 0] / (cm[0, 0] + cm[1, 0])     # 类别 0 的召回率
    ...
    # 将混淆矩阵写入 Excel
    output_file = os.path.join(args.root_path, f"info/confusion_matrix_qwen2vl-2B_{args.task_type}data.xlsx")
    ...
    for cell, text in comments.items():
        ws.cell(row=cell[0], column=cell[1]).comment = Comment(text, "Qwen2VL-2B")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="qwen2vl-2B")
    ...
    for fun_num in args.function:
        if int(fun_num) == 1:
            ...
            cal_matrix()
    print(len(os.listdir(f"/cpfs/user/wangpengfei2/project/Qwen2VL/attn_score_analysis/qwen2vl-2B/{args.task_type}/scores")))
