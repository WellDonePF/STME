...
def cal_matrix():
    ...
    for last_N in args.N:
        ...
        precision_0 = cm[0, 1] / (cm[0, 0] + cm[0, 1]) if cm[0, 0] + cm[0, 1] > 0 else 0  # 类别 0 的精度
        precision_1 = cm[1, 1] / (cm[1, 1] + cm[1, 0]) if cm[1, 1] + cm[1, 0] > 0 else 0  # answer_true & attn_true 在 answer_true 中的占比
        recall_0 = cm[1, 0] / (cm[0, 0] + cm[1, 0]) if cm[0, 0] + cm[1, 0] > 0 else 0     # 类别 0 的召回率
        recall_1 = cm[1, 1] / (cm[1, 1] + cm[0, 1]) if cm[1, 1] + cm[0, 1] > 0 else 0     # answer_true & attn_true 在 attn_true 中的占比
