...
def merge_analysis_file():
    ...
    old_marged_info = json.loads(open(f"D:/Project/python3/STME/internvl2/attn_score_analysis/{args.model_name}/{args.task_type}/info/pred.json",'r',encoding="utf-8").read())
    tmp_info = {}
...
def plot_figure(metric_values, acc):
    ...
    if args.task_type == "easy":
        title_ = "Easy Tasks"
    title_ += "   InternVL2-" + args.model_name.split("-")[-1]
    plt.title(title_, fontweight="bold", fontsize=14)
    ...
    plt.savefig(
        os.path.join(args.save_figure_path, f"metric_{args.task_type}_{args.model_name}.pdf"),
        format='pdf', dpi=3000, bbox_inches='tight')
    plt.close()  # 关闭图形以释放内存
    # open(os.path.join(args.save_file_path, f"result.json"),'w',encoding="utf-8"
    #     ).write(json.dumps(all_res, indent=4))
    print(all_res)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_path", type=str, default="D:/Project/python3/STME/internvl2/attn_score_analysis")
    parser.add_argument("--metric_path", type=str, default="D:/Project/python3/STME/internvl2/metric")
    parser.add_argument("--model_name", type=str, default="internvl2-26B")
